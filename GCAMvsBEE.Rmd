...
gcam <- gcamBau %>% filter(sector %in% common,variable=="production", year %in% c(2015:2050)) %>% select(sector,year,value,Units) %>% mutate(source="GCAM",value=if_else(sector=="Thermal Power Plant",value*277.78,value),Units = if_else(sector=="Thermal Power Plant","TWh",Units)) %>% rename("unit"=Units)
