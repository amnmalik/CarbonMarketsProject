...
ggplot(ref_join, mapping = aes(x = year.x, y = growth, color = sector, group = sector)) +
  ...
  scale_color_manual(values = jgcricol()$pal_basic) +
...
ggplot()+
  geom_line(em_sec_tot %>% filter(year > 2025, year < 2045, !scenario %in% c("NZ","NZ+RPO+ETS")),mapping =            aes(x=year,y=value,color=scenario,group=scenario))
...
p1 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, sector != "CO2", scenario %in% c("NZ+CC"))) +
...
power_ets <- em_sec %>% 
  ...
  scenario %in% c("NZ+CC", "NZ+CC_v1", "NZ+CC_v2"),year %in% c(2025:2070)) %>% mutate(value=value*3.67,Units="MTCO2") 

ets_price <- Cprices %>% 
  filter(scenario== "NZ+ETS",year %in% c(2025:2070),sector=="ETS") %>% mutate(value=value,Units="$2020/CO2")
...
revenue <- left_join(left_join(power_ets,ets_price,by="year"),scenarios,by="year") %>% mutate(rev1=value.x*value.y*scen1,rev2=value.x*value.y*scen2) %>% 
...
labs(x="",y="Billion dollars(20202$)",linetype="",color="",
