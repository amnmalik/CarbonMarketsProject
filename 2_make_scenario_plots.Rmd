...
kaya <- jgcricol()$pal_16[8:15]
names(kaya) <- c("Electricity generation","GDP per capita","transportation","industry","building","CO2 emissions","Emissions intensity (CO2/Energy)","Energy intensity (Energy/GDP)")
...
Cprices <- all_data %>%
  ...
  mutate(sector = if_else(sector == "CO2", sector, gsub(sector, pattern = "CO2", replacement = "")),
sector = mgsub::mgsub(sector,pattern = c("India","All","CO2","Elec","Fert","IRSTL","ETS"),replacement = c("", "", "CO2 (net-zero)", "Electricity", "Fertilizer", "Iron and Steel", "CO2 (ETS)")))

p1 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, scenario %in% c("NZ+CC", "NZ+ETS"))) +
...
p2 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, sector != "CO2", scenario %in% c("NZ+CC"))) +
...
library(patchwork)
p1 + inset_element(p2, left = 0.05, bottom = 0.5, right = 0.4, top = 1)
...
ij <- ggplot(fe_by_fuel_sector_share %>% filter(year %in% c(2030, 2040, 2065), sector %in% common,scenario =="NZ")) +
  ...
  scale_fill_manual(values = mypal[names(mypal) %in% unique(fe_by_fuel_sector_share$input)],
