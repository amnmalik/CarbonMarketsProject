...
kaya <- jgcricol()$pal_16[7:16]
names(kaya) <- c("Electricity generation", "GDP per capita", "transportation", "industry", "building", "CO2 emissions", "Emissions intensity (CO2/Energy)", "Energy intensity (Energy/GDP)","CO2 (net-zero)","CO2 (ETS)")

mypal <- c(kaya, techs, mypal, "delivered biomass" = "darkolivegreen2", "delivered coal" = "black", "H2" = "peachpuff2", "refined liquids industrial" = "#d01c2a")
```
...
ggplot(elec_gen_share %>% filter(year %in% c(2020, 2040, 2070), !sector %in% c("refined liquids", "geothermal"), scenario %in% c("NZ", "NZ+ETS", "NZ+RPO+ETS"))) +
...
Cprices <- all_data %>%
  ...
  mutate(sector=gsub(sector, pattern = "IndiaCO2\\b", replacement = "CO2 (net-zero)"),
    sector = mgsub::mgsub(sector, pattern = c("IndiaCO2", "All","Elec", "Fert", "IRSTL", "ETS"), replacement = c("", "", "Electricity", "Fertilizer", "Iron and Steel", "CO2 (ETS)"))
  )

p1 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, scenario %in% c("NZ+ETS"))) +
...
p2 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, scenario %in% c("NZ+ETS"))) +
  geom_line(mapping = aes(x = year, y = value, group = sector, color = sector),show.legend = F) +
  # geom_point(mapping = aes(x=year,y=value,group = scenario,shape=sector,color=sector),show.legend = F)+
  scale_color_manual(values = mypal[names(mypal) %in% unique(Cprices$sector)]) +
  theme_pubclean() +
  labs(x = "", y = "",color="") +
...
p3 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, scenario %in% c("NZ+CC"))) +
  geom_line(mapping = aes(x = year, y = value, group = sector, color = sector)) +
  # geom_point(mapping = aes(x=year,y=value,group = scenario,shape=sector,color=sector),show.legend = F)+
  facet_wrap(~scenario, shrink = T, scales = "free") +
  scale_color_manual(values = mypal[names(mypal) %in% unique(Cprices$sector)]) +
...
p4 <- ggplot(Cprices %>% filter(sector != "refining", year > 2015, year < 2070, scenario %in% c("NZ+CC"))) +
  geom_line(mapping = aes(x = year, y = value, group = sector, color = sector),show.legend = F) +
  # geom_point(mapping = aes(x=year,y=value,group = scenario,shape=sector,color=sector),show.legend = F)+
  scale_color_manual(values = mypal[names(mypal) %in% unique(Cprices$sector)]) +
  theme_pubclean() +
...
p12 <- p1 + inset_element(p2, left = 0.02, bottom = 0.5, right = 0.5, top = 0.95,ignore_tag = T)+plot_annotation(tag_levels = 'A')

p34 <- p3 + inset_element(p4, left = 0.02, bottom = 0.5, right = 0.5, top = 0.95,ignore_tag = T)+ plot_annotation(tag_levels = 'A') & labs(y="")


p1 + inset_element(p2, left = 0.02, bottom = 0.5, right = 0.5, top = 0.95,ignore_tag = T)+plot_annotation(tag_levels = 'A') + p3 + inset_element(p4, left = 0.02, bottom = 0.5, right = 0.5, top = 0.95,ignore_tag = T)+ plot_annotation(tag_levels = 'A') & guides(color = guide_legend(nrow = 2))
...
gh <- ggplot(ccs_share %>% filter(sector %in% common, year %in% c(2020, 2040, 2065, 2070), scenario == "NZ")) +
  geom_line(mapping = aes(x = year, y = perc, color = sector, group = interaction(sector, scenario)), show.legend = F) +
  geom_point(mapping = aes(x = year, y = perc, color = sector, shape = sector, group = interaction(sector, scenario)), size = 3) +
...
ij <- ggplot(fe_by_fuel_sector_share %>% filter(year %in% c(2030, 2040, 2065), sector %in% common, scenario == "NZ")) +
...
# Inter-sectoral transfers and auction revenue
```{r}
...
ggplot(transfers) +
  geom_col(mapping = aes(x = year, y = `Surplus Allowances` * 3.667, fill = Sectors, group = Sectors)) +
  scale_fill_lancet() +
  theme_pubclean() +
  facet_wrap(~scenario) +
  labs(x = "", y = " Surplus Allowances (MTCO2)", fill = "")
...
power_ets <- em_sec %>%
  filter(
    sector == "Electricity",
    scenario %in% c("NZ+CC", "NZ+CC_v1", "NZ+CC_v2"), year %in% c(2025:2070)
  ) %>%
  mutate(value = value * 3.67, Units = "MTCO2")

ets_price <- Cprices %>%
  filter(scenario == "NZ+ETS", year %in% c(2025:2070), sector == "ETS") %>%
...
revenue <- left_join(left_join(power_ets, ets_price, by = "year"), scenarios, by = "year") %>%
  mutate(rev1 = value.x * value.y * scen1, rev2 = value.x * value.y * scen2) %>%
  select(scenario.x, year, rev1, rev2) %>%
  pivot_longer(c(3:4), names_to = "scenario", values_to = "value")

ggplot() +
  geom_line(revenue %>% filter(year < 2070) %>% mutate(value = value / 1000), mapping = aes(x = year, y = value, color = scenario.x, linetype = scenario, group = interaction(scenario.x, scenario)), linewidth = 1.5) +
  labs(
    x = "", y = "Billion dollars(20202$)", linetype = "", color = "",
    subtitle = "Auctioning revenue"
  ) +
  theme_pubclean()
